...
def equity_demand(regions):
    ...
    #if the difference is 0, this can only happen if you have no cummulative patients anywhere --> no-one was treated! so instead return a high value
    if difference == 0:
        return 2
...
def equity_arrival(regions, timesteps):
    ...
    #if the difference is 0, the only way this could have happened is if you never gave infected people aid. So instead return a really high value
    if difference == 0:
        return 2000
